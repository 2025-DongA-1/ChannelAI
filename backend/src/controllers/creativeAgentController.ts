...
export const generateCreative = async (req: Request, res: Response) => {
  try {
    ...
    if (!user) return res.status(401).json(createErrorResponse(ERROR_CODES.COMMON.UNAUTHORIZED));
    ...
    if (!businessType || !productName || !targetAudience || !tone || !objective) {
      return res.status(400).json(createErrorResponse(ERROR_CODES.CREATIVE.MISSING_FIELDS));
    }
    ...
    if (documentFile) {
      ...
      if (!['.pdf', '.txt', '.md', '.csv'].some(e => ext.endsWith(e))) {
        return res.status(400).json(createErrorResponse(ERROR_CODES.CREATIVE.INVALID_FILE_TYPE, '문서 파일은 PDF, TXT, MD, CSV만 지원합니다.'));
      }
    }
    if (imageFile) {
      const ext = imageFile.originalname.toLowerCase();
      if (!['.jpg', '.jpeg', '.png', '.gif', '.webp'].some(e => ext.endsWith(e))) {
        return res.status(400).json(createErrorResponse(ERROR_CODES.CREATIVE.INVALID_FILE_TYPE, '이미지 파일은 JPG, PNG, GIF, WEBP만 지원합니다.'));
      }
    }
    ...
    return res.status(500).json(createErrorResponse(ERROR_CODES.CREATIVE.GENERATE_FAILED, error.message));
  }
};
...
export const getHistory = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json(createErrorResponse(ERROR_CODES.CREATIVE.HISTORY_FAILED, error.message));
  }
};
...
export const deleteHistory = async (req: Request, res: Response) => {
  try {
    ...
    if (isNaN(id)) return res.status(400).json(createErrorResponse(ERROR_CODES.CREATIVE.INVALID_ID));
    ...
    return res.status(500).json(createErrorResponse(ERROR_CODES.CREATIVE.DELETE_FAILED, error.message));
  }
};
...
export const getDetail = async (req: Request, res: Response) => {
  try {
    ...
    if (!detail) return res.status(404).json(createErrorResponse(ERROR_CODES.CREATIVE.NOT_FOUND));
    ...
    return res.status(500).json(createErrorResponse(ERROR_CODES.CREATIVE.DETAIL_FAILED, error.message));
  }
};
