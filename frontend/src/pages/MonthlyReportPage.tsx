...
const TAB_LABELS = ["종합 성과 현황", "채널별 분석 데이터", "기간별 성과 추이", "캠페인별 상세 성과"];
  ...
  // [2026-03-13] 현재 보고서를 PDF로 생성한 뒤, 로그인 사용자 이메일로 전송 (prompt 제거)
  const [isSendingEmail, setIsSendingEmail] = useState(false);
  const handleSendEmail = async () => {
    const email = user?.email;
    if (!email) return alert('로그인 사용자 이메일을 확인할 수 없습니다.');
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
              ...
              <div className="flex gap-2">
                ...
                <button 
                  onClick={async () => {
                    // [2026-03-13] 로그인 사용자 이메일 자동 사용 (prompt 제거)
                    const email = user?.email;
                    if (!email) return alert('로그인 사용자 이메일을 확인할 수 없습니다.');
                    setIsSendingServerEmail(true);
                    try {
                      const res = await api.post('/report/send-to', { email });
